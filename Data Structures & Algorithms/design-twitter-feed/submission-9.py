    ...
    def getNewsFeed(self, userId: int) -> List[int]:
        if userId not in self.userIDs and userId not in self.following[userId]:
            ...
            for following in self.following[userId]:
                if following != userId:
                    following_heap = self.news[following].copy()
                    while following_heap:
                        heapq.heappush(tweets, -1 * heapq.heappop(following_heap))
